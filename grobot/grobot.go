...
func NewGrobot(config string) *Grobot {
	robot := Grobot{
...
func (robot *Grobot) InitLog(config ConfigLogs) {
...
func (robot *Grobot) SendEvent(event *api.Event) {
...
func (robot *Grobot) Cron() {
	robot.LogStatistics()
	robot.Irc.AutoReconnect()
}

func (robot *Grobot) AutoJoin(s string) {
...
func (robot *Grobot) HandleNotice(serv *Server, event *api.Event) {
	switch event.CmdId {
	case 1:
...
func (robot *Grobot) HandleEvent(serv *Server, event *api.Event) {
	switch event.Type {
	case api.E_KICK:
...
func (robot *Grobot) NewModule(ac *api.Action) {
...
func (robot *Grobot) AutoRunModules() {
