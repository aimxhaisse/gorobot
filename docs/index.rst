.. m1ch3l documentation master file, created by
   sphinx-quickstart on Fri Aug 29 23:34:18 2014.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to m1ch3l's documentation!
==================================

m1ch3l, also know as gorobot, is an IRC robot with the following
features:

* multi-server
* multi-channels
* private messages
* flood control to be able to print pokemons in ASCII art without
  being G-lined
* admin commands via a dedicated set IRC channels (admin channels)
* JSON configuration
* external scripts that can be invoked via IRC (!commands)
* Docker integration

The official m1ch3l lurks in the shadows of freenode, but there are
probably some others hanging around in the wild, we don't know how
many.

Contents
========

.. toctree::
   :maxdepth: 2

   first-start
   configuration
   adding-custom-commands
   advanced-usage
   fqa

Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`

